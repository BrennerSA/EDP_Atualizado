...
class Conn(wx.Dialog):
        ...
        def __init__(self, *args, **kwargs):
                ...
                LabelTestarConexao = wx.StaticText(self, label = "TESTAR CONEXÃO", style = wx.ALIGN_CENTRE)
                LabelY1V = wx.StaticText(self, label = "Y1 (V)", style = wx.ALIGN_CENTER)
                LabelY2V = wx.StaticText(self, label = "Y2 (V)", style = wx.ALIGN_CENTER)
                LabelY1 = wx.StaticText(self, label = "Y1 (mm)", style = wx.ALIGN_CENTER)
                LabelY2 = wx.StaticText(self, label = "Y2 (mm)", style = wx.ALIGN_CENTER)

                LabelTestarConexao.SetFont(FontTitle)
                LabelY1V.SetFont(Fonttext)
                LabelY2V.SetFont(Fonttext)
                LabelY1.SetFont(Fonttext)
                LabelY2.SetFont(Fonttext)

                LabelTestarConexao.SetBackgroundColour(colorTextBackground)
                LabelY1V.SetBackgroundColour(colorTextBackground)
                LabelY2V.SetBackgroundColour(colorTextBackground)
                LabelY1.SetBackgroundColour(colorTextBackground)
                LabelY2.SetBackgroundColour(colorTextBackground)
                ...
                self.v16_sizer.Add(LabelY2, 1, wx.CENTER)
                self.v16_sizer.Add(self.y2mm, 2, wx.CENTER)

                self.v17_sizer.Add(LabelY2V, 1, wx.CENTER)
                self.v17_sizer.Add(self.y2V, 2, wx.CENTER)

                self.v18_sizer.Add(LabelY1, 1, wx.CENTER)
                self.v18_sizer.Add(self.y1mm, 2, wx.CENTER)

                self.v19_sizer.Add(LabelY1V, 1, wx.CENTER)
                ...
                self.v21_sizer.Add(LabelTestarConexao, 1, wx.CENTER)
        ...
        def LTESTE(self, event):
            ...
            cond = threadConection.getStatus()
            if cond == 'connectado':
        ...
        def Desconect(self, event):
            ...
            self.y2V.Clear()            
            time.sleep(.3)
            con.modeStoped()# sai do laço de impressão do ensaio 134
            time.sleep(.3)
            con.modeB() #sai do laço da norma 134 e retorna para o switch inicial
            time.sleep(.3)
            con.modeD() #envia mensagem D para desconectar e setar o pino de ativação para low
            try:
                self.t.join(timeout=3)
            except:
                print ('erro ao encerrar a thread')
