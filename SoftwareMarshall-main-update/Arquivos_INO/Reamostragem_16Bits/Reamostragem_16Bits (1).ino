...
Oversampling adc(10, 16, 2); //Adc Bytes 10 or 12, Oversampeling Bytes 11-18, 2 Is avaraging count.
...
//   ads.setGain(GAIN_SIXTEEN);    // 16x gain  +/- 0.256V  1 bit = 0.125mV  0.0078125mV
  ...
  float ads_InputRange_code = 3.3f;// 3.3V É a tensão máxima que pode ser lida 
...
void loop(void) 
{
  ...
  voltage_ads = adc0*ads_bit_Voltage;
  voltage_ard = analogRead(0)*ard_bit_Voltage;
  voltage_ads_code = adc.read(0)*ads_code_bit_Voltage;
  
  Serial.print(voltage_ads); Serial.print("  "); Serial.println(voltage_ads_code); 
  delay(200);
}
