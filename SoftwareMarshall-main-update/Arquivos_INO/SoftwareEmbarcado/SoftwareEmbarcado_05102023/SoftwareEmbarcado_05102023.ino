...
void loop() {
  ...
  if(conexao == 'C') calibrar_lvdts();
    ...
    lastmillis = millis(); //TEMPO INICIAL
    while((millis() - lastmillis) < timeStop){
      subir();
      ler_fluencia();
      ler_estabilidade();
      timestamp=float(millis()-lastmillis)/1000;
      if(int(timestamp*1000)%10==0){
        Serial.print(0);
        Serial.print(",");
        Serial.print(lvdt_estabilidade,4);
        Serial.print(",");
        Serial.print(timestamp,3);
        Serial.print(",");
        Serial.println(lvdt_fluencia,3); //ENVIA DADOS PARA PROGRAMA EM PYTON 
        Serial.flush();  //LIMPA BUFFER
      }                        //resistencia maxima a compressao                             deformação maxima
    ...
    Serial.print(double(0.0000),4);Serial.print(lvdt_estabilidade,4);Serial.print(0);Serial.println(lvdt_fluencia,3); //ENVIA DADOS PARA PROGRAMA EM PYTON 
    Serial.flush(); //LIMPA BUFFER 
    parar();
  }
  else if(conexao == 'F'){
...
bool emergencia(){
  ...
  if(limite_flu >= 2460.0){ // ver uma forma para não ir até o final do curso depois do inicio da força ir até delta15mm (2460 counts) 16/03
    return true;
  }
  return false;
}
